...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  const { chats, setSelectedChat, theme, setTheme, navigate, user, createNewChat,
  ...
  const deleteChat = async (e, chatId) => {
    try {
      ...
      const res = await axios.post('api/chat/delete', { chatId }, { headers: { Authorization: token } });
      if (res.data.success) {
    ...
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-sm font-semibold text-[#3D81F6]">
      ...
      <div onClick={() => { navigate('/credits'); setIsMenuOpen(false) }} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p className='text-xs text-gray-400'>Purchase credits to use SaraGPT</p>
        </div>
      </div>
      ...
      <div className='flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
        ...
        {user && <img onClick={logout} src={assets.logout_icon} className='w-4 cursor-pointer not-dark:invert
                  opacity-100 md:opacity-0 md:group-hover:opacity-100
                  transition-opacity duration-200' />}
      </div>

      <img onClick={() => setIsMenuOpen(false)} src={assets.close_icon} className='absolute w-5 h-5 top-3 right-3 cursor-pointer md:hidden not-dark:invert' alt='' />
    </div>
  )
}
