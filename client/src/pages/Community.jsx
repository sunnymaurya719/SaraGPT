...
const Community = () => {
  ...
  const fetchImages = async() =>{
    try{
      const res = await axios.get('/api/user/published-images',{headers:{Authorization:token}});
      if(res.data.success){
  ...
  console.log("Hiii guys",images.length)
          ...
          <div className='flex flex-wrap max-sm:justify-center gap-5'>
            {
              images.map((item,index)=>(
                <a key={index} href={item.imageUrl} className='relative group block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-shadow duration-300'>
